...
const ExplaoreCard = ({restaurant}) => {
    ...
    const cuisines= restaurant?.info?.cusine?.map((item)=>item.name).slice(0,3) || [];
      ...
      <div>
        <div className="explore-card-cover">
            ...
            <div className='res-row-name'>
                {  cuisines.length >0 && (<div className='res-cuisine'>
                    {cuisines.map((item,i)=>{
                       return <span className='res-cusine-tag' key={i}>{item}{i !== cuisines.length - 1 && ","}</span> 
                    })}
